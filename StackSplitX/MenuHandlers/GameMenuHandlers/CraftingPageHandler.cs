...
namespace StackSplitX.MenuHandlers
{
    public class CraftingPageHandler : GameMenuPageHandler<CraftingPage>
    {
        ...
        public override EInputHandled OpenSplitMenu(out int stackAmount)
        {
            ...
            var workbenchContainers = Helper.Reflection.GetField<List<Chest>>(this.MenuPage, "_materialContainers").GetValue();

            //Items in workbench
            List<Item> workbenchExtraIems = new List<Item>();
            for (int i = 0; i < workbenchContainers.Count; i++)
            {
                for (int j = 0; j < workbenchContainers[i].items.Count; j++)
                {
                    workbenchExtraIems.Add(workbenchContainers[i].items[j]);
                }
            }
            ...
            if (!hoveredItem.canStackWith(hoveredItem) || !(hoverRecipe.doesFarmerHaveIngredientsInInventory(extraIems)|| hoverRecipe.doesFarmerHaveIngredientsInInventory(workbenchExtraIems)))
                return EInputHandled.NotHandled;
